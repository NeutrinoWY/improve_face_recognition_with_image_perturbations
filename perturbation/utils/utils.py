...
def calculate_similarity_weighted(directories, weights, file_path):
    ...
    for i in range(n):
        ...
        features_1_mean = np.sum([weights[i] * features_1[i] for i in range(len(weights))], axis=0) / sum(weights)
        features_2_mean = np.sum([weights[i] * features_2[i] for i in range(len(weights))], axis=0) / sum(weights)
