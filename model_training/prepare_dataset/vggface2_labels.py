...
def get_val_pairs(test_labels, save_path, k=10, debug_mode=1):
    """
    ...
    NAME_1, NAME_2, LABEL = [0] * n * k * 2, [0] * n * k * 2, [0] * n * k * 2
    # generate k*n positive pairs
    i = 0 
    for class_id in list(set(class_ids)):
        print(class_id)
        samples = test_labels[test_labels.CLASS_ID.eq(class_id)]
        #print(samples)
        pairs = random.sample(samples.index.tolist(), 2*k)
        for j in range(k):
            NAME_1[i + j] = pairs[j]
            NAME_2[i + j] = pairs[k + j]
            LABEL[i + j] = 1
        i += k
    

    # generate k*n negative pairs
    count = -1
    for i in range(10000):
        ...
        if class_1 != class_2:
            count += 1
            LABEL[k * n + count] = 0
            NAME_1[k * n + count] = pair[0]
            NAME_2[k * n + count] = pair[1]
        if count == k * n - 1:
            break
